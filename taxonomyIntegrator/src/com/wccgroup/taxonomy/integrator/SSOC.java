...
*/
	...
	public static void generateRelationsWithTwoScoresData(List<Taxonomy> taxonomies, String path) throws SQLException, IOException
	{
		try
		{
			Class.forName(JDBC_DRIVER);
			...
			ResultSet rs;
			BufferedWriter writer;
			//System.out.println("\t- relations-with-two-scores"); 
			for (Taxonomy concept : taxonomies) { 
				if (!concept.getType().equalsIgnoreCase("relations-with-two-scores"))
					continue;
    ...
    private static List<Taxonomy> readTaxonomyPara(String fileName) { 
		...
		try (BufferedReader br = Files.newBufferedReader(pathToFile, StandardCharsets.US_ASCII)) { 
			...
			while (line != null) { 
				...
				if (attributes[0].equalsIgnoreCase("relations-with-two-scores")) {
					Taxonomy concept = new Taxonomy (attributes[0], attributes[1], attributes[3], null, attributes[2]); 
					taxonomies.add(concept); 
				}
